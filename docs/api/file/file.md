...
`k.file` 提供站点内**磁盘目录**上文件的操作（读写、复制、删除等）。Kooboo 后台 **没有** 与 `k.file` 一一对应的可视化文件管理页。

::: tip 与「内容 → 文件」菜单
后台 **内容 → 文件**（`/content/files`）管理的是 **CMS 文件库（CmsFile）**，可通过站点 URL 访问；与 `k.file` **不是同一套存储**，且目前 **没有** `k.xxx` API 操作 CMS 文件库。操作说明见 [后台：文件](/cms/content/files)。
:::
